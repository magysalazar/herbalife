...
Class MY_Model extends CI_Model {
    ...
    // **************** tables load ****************
    public $table_ci_options;
    public $table_ci_usuarios;
    public $table_ci_settings;
    public $table_ci_sessions;
    public $table_ci_modulos;
    ...
    public function get($id = null, $single = false){
        ...
        $result = $this->verifyArraysInResult(std2array($result));
        return $result;
    }
    private function verifyArraysInResult($result){
    ...
    public function get_by($where, $bSelecting = false, $single = false){
        ...
        foreach ($where as $k => $wh){
            if(validateVar($k,'numeric',false)){
                ...
                if(validateVar($wh,'array')){
                    ...
                    {
                        if(count($wh_interno) > 1){
                            $whereFunc = 'or_where_in';
                        }
                        if(validateVar($j,'numeric',false)){
                            $select .= ', '.$wh_interno;
                        } else if(validateVar($j,'string')) {
                            $aWheres[$j] = $wh_interno;
                        }
                    }
                } else {
                    $select .= ', '.$wh;
        ...
        foreach($aWheres as $key => $where){
            if($whereFunc == 'or_where_in'){
                $this->db->$whereFunc($key,$where);
            } else {
                $this->db->$whereFunc($where);
            }
        }
        return $this->get(null, $single);
    }
...
");
    ...
    public function save($data, $id = null, $with_id = true){
        ...
        if($this->db->field_exists('id_user_modified', $this->_table_name)){
            if($this->db->table_exists('ci_usuarios') && $this->db->field_exists('id_usuario','ci_usuarios')){
                $userAdmin = CiUsuariosQuery::create()->findOneByIdUsuario(1);
...
//    }
    ...
    public function setForeignFields($t1Contents, $t1FieldRef, $t2Contents, $t2FieldRef, $bWithAllFields = false){
        ...
        foreach ($t2Contents as $j => $t2Content){
            foreach ($t1Contents as $i => $t1Content){
                if($t2Content->$t2FieldRef == $t1Content->$t1FieldRef){
